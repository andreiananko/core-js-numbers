...
function getAverage(value1, value2) {
  switch (value1 + value2) {
    case -Infinity:
      return Number.MIN_VALUE.toExponential();
    case Infinity:
      return Number.MAX_VALUE.toExponential();
    default:
 ...
 */
function isPrime(n) {
  const sum = [...Array(n + 1).keys()].slice(1);

  return Boolean(sum.reduce((a, b) => (n % b === 0 ? a + 1 : a), 0) === 2);
}
...
function getSumOfNumbers(...args) {
  return args.reduce((a, b) => a + b, 0).toFixed(1);
}
